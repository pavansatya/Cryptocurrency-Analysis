...
# this data is from binance exchange API btc to usdt price data
df_binance = pd.read_csv('df_combined.csv')
...
df_short = df_binance[['Event time', 'Open price',
...
fig.add_trace(go.Scatter(
    x=df_binance['Event time'], y=df_binance['Open price'], mode='lines', name='Open price'))
fig.add_trace(go.Scatter(
    x=df_binance['Event time'], y=df_binance['High price'], mode='lines', name='High price'))
fig.add_trace(go.Scatter(
    x=df_binance['Event time'], y=df_binance['Low price'], mode='lines', name='Low price'))
fig.add_trace(go.Scatter(
    x=df_binance['Event time'], y=df_binance['Close price'], mode='lines', name='Close price'))
fig.show()
...
df_scaled = scaler.fit_transform(
    df_short[['High price', 'Low price', 'Open price',  'Close price']])
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        h_0 = torch.zeros(2 * self.num_layers, x.size(0),
                          self.hidden_size).to(x.device)
        c_0 = torch.zeros(2 * self.num_layers, x.size(0),
                          self.hidden_size).to(x.device)
...
predictions_df_lstm = pd.DataFrame({
    # Flatten if needed
    'Event time': df_short['Event time'].values[-len(y_pred_rescaled):].ravel(),
...
predictions_df = pd.merge(
    predictions_df, predictions_df_lstm, on='Event time', how='inner')
...
df_evaluation.loc[1] = [round(mae, 4), round(
    r2, 4), round(mape, 4), 'LSTM bidirectional']
...
fig.add_trace(go.Histogram(
    x=df_short['Close price'], name='Price Distribution'), row=1, col=2)

fig.add_trace(go.Bar(x=['Mean Absolute Error', 'R2 Score', 'Mean Absolute Percentage Error'], y=[mean_absolute_error(predictions_df['Actual'], predictions_df['Predicted']), r2_score(
    predictions_df['Actual'], predictions_df['Predicted']), mean_absolute_error(predictions_df['Actual'], predictions_df['Predicted'])/predictions_df['Actual'].mean()], name='Evaluation metrics'), row=2, col=1)
...
fig.add_trace(go.Scatter(x=predictions_df[predictions_df['Model_y'] == "LSTM bidirectional"].tail(60).sort_values(by='Event time')[
              'Event time'], y=predictions_df.tail(60).sort_values(by='Event time')['Predicted_y'], mode='lines', name='Prediction LSTM'), row=2, col=2)
fig.add_trace(go.Scatter(x=predictions_df[predictions_df['Model_y'] == "LSTM bidirectional"].tail(60).sort_values(by='Event time')[
              'Event time'], y=predictions_df.tail(60).sort_values(by='Event time')['Actual_y'], mode='lines', name='Actual price'), row=2, col=2)
fig.add_trace(go.Scatter(x=predictions_df[predictions_df['Model_x'] == "ElasticNet"].tail(60).sort_values(by='Event time')[
              'Event time'], y=predictions_df.tail(60).sort_values(by='Event time')['Predicted_x'], mode='lines', name='Prediction ElesticNet'), row=2, col=2)
fig.add_trace(go.Scatter(x=predictions_df[predictions_df['Model_x'] == "ElasticNet"].tail(60).sort_values(by='Event time')[
              'Event time'], y=predictions_df.tail(60).sort_values(by='Event time')['Actual_x'], mode='lines', name='Actual price'), row=2, col=2)
...
fig.update_layout(height=900, width=1500,
                  title_text="Time Series Forecasting Dashboard", title_x=0.5)
